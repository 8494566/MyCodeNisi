...
; CC 安装信息设置
!define PRODUCT_NAME "CC直播"
!define PRODUCT_DETAIL "CC直播"
...
!define CC_LINK_NAME "CC直播.lnk"

;显示桌面tips
!define CC_FINISHPAGE_RUN_DESKTIPS "$\"$INSTDIR\DesktopTips.exe$\""

;注册DesktopService
!define CC_FINISHPAGE_RUN_DESKSERVICE "$\"$INSTDIR\DesktopService.exe$\""
...
Var opSaveAccount
;----------卸载原因--------
Var opNoFav      ;找不到想看的直播
Var opAppCrash    ;程序崩溃、导致死机
Var opBotherOperation ;不好玩，操作麻烦
Var opHighCpu         ;CPU、内存占用高
Var opBadMultiplyMedia ;画质、音质不够好
Var opNoSmooth          ;播放不流畅、比较卡
Var postParamater
Var postJson

; 切换背景
Var unTimerID
Var unChangebkimageIndex
;获取当前磁盘剩余容量 MB
Var diskFreeMB
Var RSPACE
;Languages
...
Name      "${PRODUCT_DETAIL}"              ;提示对话框的标题 - "CC直播"
OutFile   "${PRODUCT_NAME_EN}_Setup_3.19.2_11215_gfxz.exe"    ;输出安装包名
...
Section "Uninstall"
   ...
   BEGIN_UNINSTALLATION:
    ...
    ; 删除各处的快捷方式
    Delete "$SMPROGRAMS\CC直播\Uninstall.lnk"
    Delete "$STARTMENU\CC直播.lnk"
    Delete "$DESKTOP\CC直播.lnk"
    Delete "$SMPROGRAMS\CC直播\CC直播.lnk"
    Delete "$QUICKLAUNCH\CC直播.lnk"
	Delete "$DESKTOP\直播大厅.lnk"
    RMDir /REBOOTOK /r "$SMPROGRAMS\CC直播"
...
Function CC
  ...
  nsTBCIASkinEngine::FindControl "Wizard_CloseBtn0"
   ...
   nsTBCIASkinEngine::FindControl "btn_agree"
   Pop $0
   ${If} $0 == "-1"
        MessageBox MB_OK "Do not have btn agree button"
   ${Else}
        GetFunctionAddress $0 OnBtnAgree
	nsTBCIASkinEngine::OnControlBindNSISScript "btn_agree" $0
   ${EndIf}
   
   ; Option - 同意CC直播的用户许可协议
  ...
  nsTBCIASkinEngine::FindControl "edit_path"
   ...
   ;切换背景绑定函数
   nsTBCIASkinEngine::FindControl "lbl_poster"
   Pop $0
   ${If} $0 == "-1"
	     MessageBox MB_OK "Do not have lbl_poster button"
   ${Else}
        nsTBCIASkinEngine::SetControlData "lbl_poster_u" "poster_1.png" "bkimage"
        StrCpy $changebkimageIndex  "0"
   ...
   ${EndIf}
   ;运行CC直播按键
   nsTBCIASkinEngine::FindControl "btn_run_cc"
   Pop $0
   ${If} $0 == "-1"
	    MessageBox MB_OK "Do not have btn_run_cc button"
   ${Else}
	    GetFunctionAddress $0 OnBtnRunCC
    	nsTBCIASkinEngine::OnControlBindNSISScript "btn_run_cc" $0
   ${EndIf}
...
Function un.CCUninstall
  ...
  ;切换背景绑定函数
   nsTBCIASkinEngine::FindControl "lbl_poster_u"
   Pop $0
   ${If} $0 == "-1"
	     MessageBox MB_OK "Do not have lbl_poster_u button"
   ${Else}
        StrCpy $unChangebkimageIndex  "0"
				GetFunctionAddress $unTimerID un.OnChangePosterFunc   
				nsTBCIASkinEngine::TBCIACreatTimer $unTimerID 2000  ;callback interval        
   ${EndIf}   
    ;--------------------------------卸载完成页面----------------------------------------
    ;关闭按钮绑定函数 --- 右上角
...
Function OnGlobalCancelFunc
     nsTBCIASkinEngine::TBCIASendMessage $Dialog WM_TBCIACANCEL "3" '你确实要退出"CC直播"安装程序？'
     Pop $0
...
Function OnBtnAgree
 nsTBCIASkinEngine::SetControlData "op_agree_licence" "false" "optioncheck"
 ${If} $vOpAgreeLicence == 0
  nsTBCIASkinEngine::SetControlData "btn_quick_install" "true" "visible"
  nsTBCIASkinEngine::SetControlData "lbl_quick_install" "false" "visible"
   StrCpy $vOpAgreeLicence 1
...
Function OnBtnShowLicence
    nsTBCIASkinEngine::TBCIASendMessage $Dialog WM_TBCIAOPENURL "http://cc.163.com/agreement/"
FunctionEnd
...
;------------------第二个页面
Function OnChangePosterFunc
   ${If} $changebkimageIndex == "0"
        StrCpy $changebkimageIndex "1"
	nsTBCIASkinEngine::SetControlData "lbl_poster" "poster_1.png" "bkimage"
   ${ElseIf} $changebkimageIndex == "1"
         StrCpy $changebkimageIndex "0"
	nsTBCIASkinEngine::SetControlData "lbl_poster" "poster_2.png" "bkimage"
   ;${ElseIf} $changebkimageIndex == "2"
    ;        StrCpy $changebkimageIndex "0"
   ;	nsTBCIASkinEngine::SetControlData "lbl_poster" "poster_3.png" "bkimage"
   ;${ElseIf} $changebkimageIndex == "3"
            ;StrCpy $changebkimageIndex "0"
	;nsTBCIASkinEngine::SetControlData "lbl_poster" "poster_4.png" "bkimage"
   ${EndIf}
...
Function OnBtnRunCC
nsTBCIASkinEngine::SetControlData "op_run_cc" "false" "optioncheck"
 ${If} $vOpRunCC == 1
     StrCpy $vOpRunCC 0
     nsTBCIASkinEngine::SetControlData "btn_start_now" "false" "visible"   ; 显示
     nsTBCIASkinEngine::SetControlData "btn_install_complete" "true" "visible"   ; 显示
...
Function OnBtnInstallComplete
  Exec '"$INSTDIR\CC.exe" -installlog -autorun $vOpRunAuto -runccnow $vOpRunCC'
  ExecShell Open "${CC_FINISHPAGE_RUN_DESKSERVICE} -i" SW_HIDE
  StrCmp $vOpRunAuto "1" "" +2
 WriteRegStr ${PRODUCT_AUTORUN_ROOT_KEY} "${PRODUCT_AUTORUN_KEY}" "${PRODUCT_AUTORUN_VALUE}" "$\"$INSTDIR\Start.exe$\""
 nsTBCIASkinEngine::TBCIASendMessage $Dialog WM_TBCIAFINISHEDINSTALL
FunctionEnd
...
Function OnStartInstallBtnFunc
   ${If} $diskFreeMB < 130
    nsTBCIASkinEngine::TBCIASendMessage $Dialog WM_TBCIACANCEL "2" "磁盘空间不足，请删除部分文件；或更改安装位置！"
    return
   ${EndIf}
   nsTBCIASkinEngine::TBCIASendMessage $Dialog WM_TBCIASTARTINSTALL
   GetFunctionAddress $timerID OnChangePosterFunc
   StrCpy $changebkimageIndex "1"
   ExecShell Open "${CC_FINISHPAGE_RUN_DESKSERVICE} -s" SW_HIDE
   KillProcDLL::KillProc "QDesktopTips.exe"
   nsTBCIASkinEngine::TBCIACreatTimer $timerID 3500  ;callback interval   
FunctionEnd

Function BuildShortCut

    ; 开始菜单
    CreateDirectory "$SMPROGRAMS\CC直播"
    CreateShortCut "$SMPROGRAMS\CC直播\CC直播.lnk" "$\"$INSTDIR\Start.exe$\""
    CreateShortCut "$SMPROGRAMS\CC直播\卸载CC直播.lnk" "$\"$INSTDIR\uninstall.exe$\""
    CreateShortCut "$STARTMENU\CC直播.lnk" "$\"$INSTDIR\Start.exe$\""
	CreateShortCut "$DESKTOP\直播大厅.lnk" "$\"$INSTDIR\LiveStart.exe$\""
...
Function un.OnGlobalCancelFunc
   nsTBCIASkinEngine::TBCIASendMessage $Dialog WM_TBCIACANCEL "3" "你确实要退出CC直播卸载程序?"
   Pop $0
   ${If} $0 == ${ID_YES}
     nsTBCIASkinEngine::ExitTBCIASkinEngine
   ${EndIf}
...
Function un.OnBtnUninstall
   nsTBCIASkinEngine::TBCIASendMessage $Dialog WM_TBCIACANCEL "3" "系统将卸载您本机安装的CC直播，是否继续？"
FunctionEnd

Function un.OnStartUninstallBtnFunc
  ;messagebox MB_OK "start uninstall"
  nsTBCIASkinEngine::TBCIASendMessage $Dialog  WM_TBCIACANCEL "4" "系统将卸载您本机安装的CC直播，是否继续?"
  pop $0
  ${If} $0 == ${ID_NO}
      nsTBCIASkinEngine::ExitTBCIASkinEngine 
  ${Else}
	  ;删除DesktopService
	  KillProcDLL::KillProc "QDesktopTips.exe"
	  ExecShell Open "${CC_FINISHPAGE_RUN_DESKSERVICE} -d" SW_HIDE
  ...
  ${EndIf}
   ...
   StrCpy $RSPACE " "
   StrCpy $postJson {"noFavourite":$opNoFav,"appCrash":$opAppCrash,"noFun":$opBotherOperation,"highCpu":$opHighCpu,"badMedia":$opBadMultiplyMedia,"noSmooth":$opNoSmooth,"uninstallTime":$2-$1-$0$RSPACE$4:$5:$6}
   ;MessageBox MB_OK $postJson
    StrCpy $postParamater "type=uninstall&reason=$postJson"
    inetc::get   "http://userlog.cc.netease.com/query?$postParamater" /END
   Pop $0
FunctionEnd
...
Function un.AfterUnistall
	IfErrors 0 QUERY_REBOOT
	       nsTBCIASkinEngine::TBCIASendMessage $Dialog WM_TBCIACANCEL "1" "部分文件无法删除，可能需要您手动清除。"
	QUERY_REBOOT:
   IfRebootFlag 0 NO_REBOOT
	    ...
	    ${EndIf}
	NO_REBOOT:
	    nsTBCIASkinEngine::TBCIASendMessage $Dialog WM_TBCIACANCEL "1"  '"CC直播"已经从您的计算机成功卸载'
...
Function un.OnChangePosterFunc
   ${If} $unChangebkimageIndex == "0"
        StrCpy $unChangebkimageIndex "1"
	      nsTBCIASkinEngine::SetControlData "lbl_poster_u" "poster_1.png" "bkimage"
   ${Else}
        StrCpy $unChangebkimageIndex "0"
	      nsTBCIASkinEngine::SetControlData "lbl_poster_u" "poster_2.png" "bkimage"
   ${EndIf}
FunctionEnd
