...
int CRichEditUI::GetFirstVisibleLine() const
{
	if( !m_pTwh ) return 0;
	LRESULT lResult;
...
void CRichEditUI::ScrollEnd()
{
	if( !m_pTwh ) return ;
	LRESULT lResult;
	TxSendMessage(WM_VSCROLL, SB_BOTTOM, NULL, &lResult);
}
...
BYTE s_byAlphaBack[MAX_ALPHABUF];

LPBYTE ALPHABACKUP(BITMAP *pBitmap,int x,int y,int cx,int cy)
{
...
//恢复位图的Alpha通道
void ALPHARESTORE(BITMAP *pBitmap,int x,int y,int cx,int cy,LPBYTE lpAlpha)
{
	if(x+cx>=pBitmap->bmWidth) cx=pBitmap->bmWidth-x;
	if(y+cy>=pBitmap->bmHeight) cy=pBitmap->bmHeight-y;
	if(cx<0 || cy<0) return;
...
// 			if ((*lpBits) == 0  && (*(lpBits - 1) != 0 || *(lpBits - 2) != 0 || *(lpBits - 3) != 0))
// 			{
// 				bb = *lpBits;
// 				aa = lpAlpha[iRow*cx+iCol];
// 			}
// 			else
// 			{
// 				*lpBits=lpAlpha[iRow*cx+iCol];
...
BOOL AlphaBackup(HDC hdc,LPCRECT pRect,ALPHAINFO &alphaInfo)
{
...
void AlphaRestore(ALPHAINFO &alphaInfo)
{
...
IRichEditOle* CRichEditUI::getRichEditOle()
{
	...
	GetTextServices()->TxSendMessage(EM_GETOLEINTERFACE, 0, (LPARAM)&m_pRichEditOle, NULL);
