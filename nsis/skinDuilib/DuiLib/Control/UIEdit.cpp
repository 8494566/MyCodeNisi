...
namespace DuiLib
{
	class CEditWnd : public CWindowWnd, public CAccelerator
	{
	public:
		CEditWnd();
	...
	{
		m_pOwner = pOwner;
		pOwner->GetHotKey(m_wModifier, m_wVK);
		RECT rcPos = CalPos();
		UINT uStyle = 0;
	...
	LRESULT CEditWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
	{
		...
		if (m_pOwner->GetHotKeyMode())
		{
			if (uMsg == WM_KEYDOWN)
			{
				...
				BOOL bBack = GetKeyState(VK_BACK)&0x8000;
				if (bBack)
				{
					m_wModifier = 0;
					m_wVK = 0;
				...
				{
					SStringT strKey=GetKeyName(TCHAR(wParam));
					if (!strKey.IsEmpty())
					{
						m_wVK = TCHAR(wParam);		
						if (bAlt)
						{
							m_wModifier |= HOTKEYF_ALT;
						}
						else
						{
							m_wModifier &= ~(HOTKEYF_ALT);
			...
			else if (uMsg == WM_KEYUP)
			{
				...
				m_pOwner->SetHotKey(m_wModifier, m_wVK);
				m_pOwner->SetText(FormatHotkey());
			}
		}
	...
	void CEditUI::SetHotKeyMode(bool isHotKeyMode/* = false*/)
	{
		m_bHotKeyMode = isHotKeyMode;
	}

	bool CEditUI::GetHotKeyMode()
	{
		return m_bHotKeyMode;
	}

	void CEditUI::SetHotKey(WORD wModifier, WORD wVK)
	{
		m_wModifier = wModifier;
		m_wVK = wVK;
	}

	void CEditUI::GetHotKey(WORD &wModifier, WORD &wVK)
	{
		wModifier = m_wModifier;
		wVK = m_wVK;
	}
}
