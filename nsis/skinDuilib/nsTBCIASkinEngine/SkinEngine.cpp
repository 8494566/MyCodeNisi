...
// 		return 0;
	...
	case WM_TBCIASTARTUNINSTALL: lRes = OnTBCIAStartInstallMSG(uMsg, wParam, lParam, bHandled); break;
	...
	void CTBCIAMessageBox::Notify(TNotifyUI& msg)
	{
		...
		if( _tcsicmp( msg.sType, _T("click") ) == 0 ){
			if( iter != m_controlCallbackMap.end() )
				...
				printf("closebtn click!!!!!!!!!!!!!!!!\n");
			}
			else if( _tcsicmp( msg.pSender->GetName(), g_messageBoxYESBtnControlName ) == 0 ){
				int wpara = 1;
				COptionUI* pOption = static_cast<COptionUI*>(this->GetPaintManager().FindControl( _T("op_save_account") ));
				if( pOption != NULL )
				{
					printf("find option!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
					wpara =  !pOption->IsSelected() + 3;  // Ñ¡ÖÐ×´Ì¬
				}
				SetParent( this->GetHWND(), NULL );
				ShowWindow( SW_HIDE, FALSE );
				::EnableWindow(g_pFrame->GetHWND(), TRUE);
				...
				printf("yesbtn click!!!!!!!!!!!!!!!!\n");
				this->ShowWindow(false);
				::PostMessage(this->GetHWND(),WM_CLOSE,wpara,0);				
			}
				...
				//Close(1);
				this->ShowWindow(false);
				::PostMessage(this->GetHWND(),WM_CLOSE,2,0);
			}
			//::PostMessage(this->GetHWND(),WM_QUIT,2,2);
		}
 		else if( _tcsicmp( msg.sType, _T("textchanged") ) == 0 ){
 			if( iter != m_controlCallbackMap.end() )
 				g_pluginParms->ExecuteCodeSegment( iter->second - 1, 0 );
	...
	LRESULT CTBCIAMessageBox::OnMsgBox(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
	{
	...
	case WM_SIZE:          lRes = OnSize(uMsg, wParam, lParam, bHandled); break;
	case WM_SYSCOMMAND:    lRes = OnSysCommand(uMsg, wParam, lParam, bHandled); break;
	case WM_MSGBOX:        lRes = OnMsgBox(uMsg, wParam, lParam, bHandled); break;
	default:
		bHandled = FALSE;
		}
		if( bHandled ) return lRes;
